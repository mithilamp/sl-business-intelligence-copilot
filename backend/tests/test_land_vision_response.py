...
def test_land_agent_traced_steps_preserve_analysis_flow():
    ...
    agent.parser.parse.return_value = ["survey.png"]
    agent.vision = Mock()
    agent.vision.analyze.return_value = (
        '{"location_information": ["Kandy, Sri Lanka"]}'
    )
    agent.location_normalizer = Mock()
    agent.location_normalizer.normalize.return_value = {
        "search_query": "Kandy, Sri Lanka",
        "confidence": "high",
    }
    agent.geo = Mock()
    agent.geo.locate.return_value = {"found": True}
    agent.business_analyzer = Mock()
    agent.business_analyzer.analyze.return_value.model_dump.return_value = {
        "opportunities": []
    }
    agent.report_builder = Mock()
    agent.report_builder.build.return_value.model_dump.return_value = {
        "property_summary": {}
    }
    ...
    agent.location_normalizer.normalize.assert_called_once_with(
        ["Kandy, Sri Lanka"]
    )
    agent.geo.locate.assert_called_once_with(
        "Kandy, Sri Lanka",
    ...
    assert result["analysis"][0]["geolocation"] == {"found": True}
